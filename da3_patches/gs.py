...
def export_to_gs_video(
    ...
    gs_world = prediction.gaussians
    # Prune scale floaters before rendering (keeps full SH coefficients intact)
    _log_scales = gs_world.scales.clamp(min=1e-8).log()  # (1, N, 3)
    _max_log_scale = _log_scales.max(dim=-1).values.squeeze(0)  # (N,)
    _keep = _max_log_scale <= 0.5
    from depth_anything_3.specs import Gaussians as _Gaussians
    gs_world = _Gaussians(
        means=gs_world.means[:, _keep],
        scales=gs_world.scales[:, _keep],
        rotations=gs_world.rotations[:, _keep],
        harmonics=gs_world.harmonics[:, _keep],
        opacities=gs_world.opacities[:, _keep],
    )
    ...
    color, depth = run_renderer_in_chunk_w_trj_mode(
        ...
        trj_mode=trj_mode,
        use_sh=True,
        max_sh_degree=1,  # cap degree-3 SH to suppress high-freq color flicker
