...
def render_3dgs(
    ...
    use_sh: bool = True,
    max_sh_degree: Optional[int] = None,  # cap SH degree to suppress high-freq color flicker
    num_view: int = 1,
    ...
    **kwargs,
    ...
    if use_sh:
        ...
        degree = isqrt(n) - 1
        if max_sh_degree is not None and max_sh_degree < degree:
            degree = max_sh_degree
            n = (degree + 1) ** 2
            gaussian_sh_coefficients = gaussian_sh_coefficients[:, :, :, :n]
...
def run_renderer_in_chunk_w_trj_mode(
    ...
    **kwargs,
    ...
    if trj_mode == "original":
        ...
        tgt_intr = intr_normed
    elif trj_mode == "smooth_light":
        # Light stabilization (k=11) without wander/dolly -- avoids SH color explosions
        tgt_c2w = _smooth_trj_fn_batch(cam2world, k_size=11)
        tgt_intr = intr_normed
