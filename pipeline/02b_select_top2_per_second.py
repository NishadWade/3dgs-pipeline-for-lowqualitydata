"""
From every 5-frame window (1 second of 30fps video sampled every 5 frames),
pick the TOP 2 frames by: sharpness x exposure_quality.

Input:  da3_output_iphone_v8_frames  (5 frames/second, 1531 total)
Output: da3_output_iphone_v10_best   (2 frames/second, ~510 total)

Naming: sequential {prefix}_{N:04d}.jpg within each prefix.
"""
import cv2
import numpy as np
import os
import shutil
from pathlib import Path

SRC_DIR        = r"D:\3DGS_Project\da3_output_iphone_v8_frames"
DST_DIR        = r"D:\3DGS_Project\da3_output_iphone_v10_best"
FRAMES_PER_SEC = 5   # exactly 5 per second (every-5-frame extraction from 30fps)
TOP_N          = 2

os.makedirs(DST_DIR, exist_ok=True)

def sharpness(img):
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    return cv2.Laplacian(gray, cv2.CV_64F).var()

def exposure_score(img):
    mean = img.mean()
    if mean < 15:
        return 0.0
    if mean > 235:
        return 0.0
    return 1.0 - abs(mean - 128) / 128

src = Path(SRC_DIR)
total_saved = 0

for prefix in ["v1", "v2", "v3", "v4"]:
    files = sorted(src.glob(f"{prefix}_*.jpg"))
    if not files:
        continue

    n = len(files)
    n_windows = (n + FRAMES_PER_SEC - 1) // FRAMES_PER_SEC
    saved = 0
    sharpness_scores = []

    for w in range(n_windows):
        window = files[w * FRAMES_PER_SEC : (w + 1) * FRAMES_PER_SEC]

        scored = []
        for f in window:
            img = cv2.imread(str(f))
            if img is None:
                continue
            sharp = sharpness(img)
            exp   = exposure_score(img)
            score = sharp * (exp + 0.1)
            scored.append((score, sharp, f))

        scored.sort(key=lambda x: x[0], reverse=True)
        picks = scored[:TOP_N]

        # Sort picks by original filename order so temporal order is preserved
        picks.sort(key=lambda x: x[2].name)

        for score, sharp, f in picks:
            saved += 1
            total_saved += 1
            dst_name = f"{prefix}_{saved:04d}.jpg"
            shutil.copy2(str(f), os.path.join(DST_DIR, dst_name))
            sharpness_scores.append(sharp)

    print(f"[{prefix}] {n} frames -> {saved} top-2-per-second  "
          f"(sharpness: mean={np.mean(sharpness_scores):.0f}  min={np.min(sharpness_scores):.0f})")

print(f"\nTotal selected: {total_saved} -> {DST_DIR}")
